...
import com.changtian.factor.entity.PeriodsSetting;
...
import com.changtian.factor.web.entity.ProjectTraConfigKeyGroupVo;
import com.changtian.factor.web.entity.ProjectTraPropertyStrategyInputVo;
...
import com.changtian.factor.web.mapper.FactorFxOhlcHisMapper;
import com.changtian.factor.web.mapper.FactorFxSampleTickDataMapper;
...
@Component(Constants.TA)
public class ForexConfigHandler implements StrategyConfigHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());
    @Resource
    private FactorFxOhlcHisMapper factorFxOhlcHisMapper;
    @Resource
    private FactorFxSampleTickDataMapper factorFxSampleTickDataMapper;
    @Autowired
        ...
        for (GlobalParameter globalParameter : response.getGlobalParameter()) {
            if (globalParameter.getXpoint1() != null && globalParameter.getXpoint2() != null) {
                ...
                if (!response.getPeriodsSetting().getOhlc().equals(Constants.TICK)) {
                    int hasBarStart = factorFxOhlcHisMapper.selectHasBar(response.getMarketDataSetting().getLeg(),
                            response.getPeriodsSetting().getOhlc(), DateUtil.format(startTime, DateUtil.YYYYMMDD_HH_MM_SS));
                    int hasBarend = factorFxOhlcHisMapper.selectHasBar(response.getMarketDataSetting().getLeg(),
                            response.getPeriodsSetting().getOhlc(), DateUtil.format(endTime, DateUtil.YYYYMMDD_HH_MM_SS));
                    if (hasBarStart <= 0 || hasBarend <= 0) {
                        x2Num = 0;
                        x3Num = 0;
                    } else {
                        x2Num = factorFxOhlcHisMapper.selectBarNum(response.getMarketDataSetting().getLeg(), response.getPeriodsSetting().getOhlc(),
                                DateUtil.format(startTime, DateUtil.YYYYMMDD_HH_MM_SS), DateUtil.format(endTime, DateUtil.YYYYMMDD_HH_MM_SS));
                        x3Num = factorFxOhlcHisMapper.selectBarNum(response.getMarketDataSetting().getLeg(), response.getPeriodsSetting().getOhlc(),
                                DateUtil.format(startTime, DateUtil.YYYYMMDD_HH_MM_SS), DateUtil.format(nowTime, DateUtil.YYYYMMDD_HH_MM_SS));
                    }
                } else {
                    int hasBarStart = factorFxSampleTickDataMapper.selectTickInfo(response.getMarketDataSetting().getLeg(),
                            DateUtil.format(startTime, DateUtil.YYYYMMDD_HH_MM_SS));
                    int hasBarend = factorFxSampleTickDataMapper.selectTickInfo(response.getMarketDataSetting().getLeg(),
                            DateUtil.format(endTime, DateUtil.YYYYMMDD_HH_MM_SS));
                    if (hasBarStart <= 0 || hasBarend <= 0) {
                        x2Num = 0;
                        x3Num = 0;
                    } else {
                        x2Num = factorFxSampleTickDataMapper.selectTickBarNum(response.getMarketDataSetting().getLeg(),
                                startTime.getTime(), endTime.getTime());
                        x3Num = factorFxSampleTickDataMapper.selectTickBarNum(response.getMarketDataSetting().getLeg(),
